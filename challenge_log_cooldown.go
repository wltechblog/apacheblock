...
// isChallengeLoggedIP checks if an IP address is currently in the logged IPs map.
func isChallengeLoggedIP(ip string) bool {
	challengeLoggedIPsMutex.Lock()
	defer challengeLoggedIPsMutex.Unlock()
	
	expiry, exists := challengeLoggedIPs[ip]
	if exists && time.Now().Before(expiry) {
		// IP is in the map and not expired
		if debug {
			log.Printf("IP %s found in challenge logged IPs (expires %s)", 
				ip, expiry.Format(time.RFC3339))
		}
		return true
	}
	
	// IP is not in the map or is expired
	return false
}
